var Individual = function(_rad, _pos, _index){
	...
	this.display = function(){
		noFill();
		stroke(this.col);
		fill(color(red(this.col), green(this.col), blue(this.col), 100));
		strokeWeight(1);
		push();
		translate(this.pos.x, this.pos.y);
		rotate(this.velocity.heading());
		for(var i = 0; i < 10; i++){
			ellipse(noise(millis()*0.0005, this.index+i)*this.spread, noise(this.index+i, millis()*0.0005)*this.spread, this.rad+cos(millis()*0.005+i*10), this.rad+cos(millis()*0.005+i*10));
		}
		...
		// strokeWeight(1);
		// for(var i = 0; i < individuals.length; i++){
		// 	if(abs(this.accepted - individuals[i].getType()) < 0.5)
		// 		line(this.pos.x, this.pos.y, individuals[i].getPos().x, individuals[i].getPos().y);
		// }
	}
